...
export default function TRDevPage() {
  const [data, setData] = useState<LineData | null>(null);
  const [loading, setLoading] = useState(true);
  const [page, setPage] = useState(1);

  useEffect(() => {
    fetch(`/api/transactions?lineOfService=TR_DEV&page=${page}&limit=25`)
      .then((res) => res.json())
      .then(setData)
      .catch(console.error)
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">TR Dev</h1>
        <p className="text-sm text-slate-500 mt-1">Software development and engineering services</p>
      </div>
